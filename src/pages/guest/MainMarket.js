...
export default class Market extends Component {
    ...
    async _loadData() {
        ...
        const forSalePunks = await Api.get('/all')
        ...
        await CT.storage.PUNKS.map(async punk => {
            forSalePunks.map(forSalePunk => {
                currentPunkIndex = forSalePunk.token_id
                currentPunkIndex == punk.idx && punks.push({
                    ...punk,
                    extra: {
                        punkBids: forSalePunk.data.punkBids ?? {},
                        punksOfferedForSale: forSalePunk.data.punksOfferedForSale ?? {},
                    }
                })
            })
        })
    ...
    async filterOnlyHasBid(e) {
        ...
        if (e.checked) {
            ...
            this._sst('interf', { isOnlyOffered: false })
        }
    }
    async filterOnlyHasOffer(e) {
        this._sst('interf', { isLoading: true })
        const [CT] = this.context;
        const punks = this.state.storage.punks
        let punksToShow = []
        ...
        if (e.checked) {
            await punks.map(async punk => {
                if (punk.extra.punksOfferedForSale[0]) {
                    punksToShow.push(punk)
                }
            })
            await this._sst('storage', { modify_punksToShow: punksToShow })
            this._sst('interf', { isLoading: false })
            this._sst('interf', { isOnlyHasBid: false })
            this._sst('interf', { isOnlyOffered: true })
        } else {
            this.filterSortByRanks({ checked: false })
            this.filterOnlyHasBid({ checked: false })
            await this._sst('storage', { modify_punksToShow: null })
            await this._sst('storage', { punksToShow: punks })
            this._sst('interf', { isLoading: false })
            this._sst('interf', { isOnlyHasBid: false })
            ...
            <>
                <div className="explorer">
                    <div className="filtering">
                        ...
                        <div className="extra">
                            {(!interf.isOnlyHasBid && !interf.isOnlyOffered) &&
                            ...
                            {(!interf.isOnlyOffered) &&
                            <div className="fast_filter">
                                <label onClick={(e) => this.filterOnlyHasBid(e.target)} className="CheckBox">Bids
                                    <input type="checkbox"/>
                                    <span className="checkmark"></span>
                                </label>
                            </div>
                            ...
                            {(!interf.isOnlyHasBid) &&
                            <div className="fast_filter">
                                <label onClick={(e) => this.filterOnlyHasOffer(e.target)} className="CheckBox">Offered
                                    <input type="checkbox"/>
                                    <span className="checkmark"></span>
                                </label>
                            </div>
